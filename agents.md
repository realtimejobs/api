...
- [OpenAPI](openapi.yaml): machine-readable customer HTTP request, response, and error schemas. Its production server URL is `https://rtj.app`.
...
- For failures, use HTTP status and the [retry guide](errors.md#retry-safely). Stop on authentication errors. Do not retry indefinitely or assume retries are free.
